...
def fetch_price(producer, symbol):
    ...
    try:
        ...
        print stock[0]['LastTradePrice']
        price = float(stock[0]['LastTradePrice'])

        #price = random.randint(80, 120)

        timestamp = datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%dT%H:%MZ')
        payload = ('[{"StockSymbol":"%s","LastTradePrice":%f,"LastTradeDateTime":"%s"}]' % (symbol, price, timestamp)).encode('utf-8')
